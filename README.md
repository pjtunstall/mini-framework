...
The key players in my framework are `VNode`s. The obvious next step would be to integrate components (i.e. functions that evaluate to `VNode`s) as a third type of node in the virtual DOM, along with the currently implemented element nodes and text nodes. Rather than letting event handlers modify the virtual DOM directly, I could restrict event handlers to modifying state variables (whether global or local to a component). A change in a state variable, could trigger components that rely on it to be called and the `VNode` that each returns to be appended.
...
<a id="f5" href="#ref-f5">5</a>: An unanticipated effect of this practice of directly modifying the virtual DOM inside of event handlers was that, when I first introduced state management via a proxy object, I wouldn't see all changes I expected till the following user interaction, leading to accumulating inconsistencies. On AI advice, I placed the update function, with its diff and reconciliation, in a `requestAnimationFrame` callback, and that worked. Only later did I guess the reason. It turns out that proxy traps are called synchronously! (I'd assumed they were asynchronous.) To see why this caused the issue, consider an event handler that first modifies a state variable, then directly modifies the virtual DOM, and then returns without any more changes of state variables. With synchronous updates, the diff and reconciliation process happens immediately, as soon as the state variable changes, so it can't include the subsequent changes made to the virtual DOM. These won't show up till the next state change, which will only be triggered by further user interaction. [↩](#ref-f5)
